...
help:                      # Выводит список доступных команд
	...
	@echo "  make ii         - Запустить скрипт парсера структуры проекта, затем запустить парсер кода."
...
.PHONY: dev
dev:
	$(NPM) run dev  		# Запускает фронтенд через Vite для локальной разработки

.PHONY: server
server:                     # Запускает сервер напрямую без Docker (использует ENV)
	$(NPM) run server:dev

# Запуск фронтенда через Vite
.PHONY: ii                # Объявляем ii как "phony"
ii:                       # Запускает скрипт парсера структуры проекта, затем запускает парсер кода.
	$(NPM) run build:path 
	-$(NPM) run build:code 
